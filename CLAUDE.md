...
**Migration (temporary, delete after everyone has the update).** `pullFromServer` copies the user's old server-side holdings into local storage once, on first launch after the update. Four rules keep it from eating data: it retries on every launch until the server actually answers (an offline first launch must not count); whatever is already on the phone always wins over the server copy; the old `sekeh_holdings_draft_v1:<userId>` draft outranks the server value (it means "typed but never uploaded") and is imported even when the server is unreachable; and a 404 marks the migration done, because it means `/api/holdings` has been removed. `mobile/src/api/holdings.ts` exists only for this and goes away with the endpoint.

**Then `clearServerCopy` empties the server's copy** — `PUT /api/holdings` with `quantity: 0, avgBuyPrice: null` for the keys that had data. Deliberately the existing endpoint rather than a new `DELETE`: it needs no backend change and no VPS deploy, and the empty rows disappear anyway when the table is dropped. It retries until it succeeds, so **`holdingCount`/`totalHoldingsValue` on the admin overview falling to zero is the signal that every install has migrated** and the table can be dropped — otherwise there is no way to know, and dropping early would destroy the data of anyone who hadn't opened the app yet.

This is the one irreversible step, so it is gated three ways: the migration flag (set only after a successful pull), `verifyStored` (the merged holdings are read back off the disk and compared before anything is wiped), and for installs that migrated before this logic existed, a check that every key about to be zeroed is present locally. The keys to wipe are recorded in `sekeh_holdings_wipe_pending_v1:<userId>` at migration time, because that is the only moment we know exactly what was written.
