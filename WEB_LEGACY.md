# دفتر دارایی — طلا، سکه و صندوق (نسخه‌ی قدیمی، سایت استاتیک)

> این مستند مربوط به نسخه‌ی اول پروژه‌ست: یک سایت استاتیک ساده روی GitHub Pages
> (فایل‌های `index.html`، `css/`، `js/`، `data/`، `scripts/` در ریشه‌ی همین ریپو).
> نسخه‌ی فعلی پروژه یک بک‌اند واقعی + اپ موبایل است — به [README.md](./README.md) نگاه کن.
> این سایت استاتیک همچنان روی GitHub Pages فعال می‌مونه مگه اینکه خودت غیرفعالش کنی.
